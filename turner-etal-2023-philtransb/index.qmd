...
group_prevalence <- function(trial, tracked_behaviors = c("Legacy", "Adaptive")) {
  ...
  # We make sure that all triples for both Behavior col factors, i.e., there is a 
  # row for every triple of `t`, `Group`, and `tracked_behaviors`. There is some
  # magic under the tidy hood that uses knowledge of each triple entry's underlying
  # factor structure; in this case 
...
plot_by_group_prevalence <- function(trial, tracked_behaviors = c("Legacy", "Adaptive")) {
...
abm <- make_minmaj_structmix(adaptive_fitness = 1.2, homophily = 0.5)
trial <- socmod::run_trial(abm, stop = socmod::fixated)
# Plot prevalence for each behavior, with line style indicating group-level prevalence.
p <- plot_by_group_prevalence(trial, tracked_behaviors = c("Legacy", "Adaptive"))
print(p)
```
...
```{r}
p <- plot_by_group_prevalence(trial, tracked_behaviors = "Adaptive")
print(p)
```
